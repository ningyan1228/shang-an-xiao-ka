...
export function AppHeader() { return <header><NavLink className="brand" to="/"><span className="brand-mark">学</span><b>上岸小卡</b><em>考公常识视觉记忆系统</em></NavLink><nav>{links.map(({to,label}) => <NavLink key={to} to={to}>{label}</NavLink>)}</nav><NavLink className="account-link" to="/account">我的账户</NavLink><ThemeToggle/></header>; }
