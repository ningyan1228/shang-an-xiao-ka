...
export function QuickCardPage() {
  ...
  return <section className="admin-page quick-card-page">
    ...
    <form className="admin-form" onSubmit={submit}>
      ...
      <label>学习讲解（学习页展示）<textarea value={form.mnemonic ?? ''} placeholder="用通俗的话讲清知识点；不要写 A、B、C、D 选项。" onChange={event => set('mnemonic', event.target.value || null)} /></label>
      <label>答题解析（确认答案后展示）<textarea value={form.explanation} onChange={event => set('explanation', event.target.value)} placeholder="解释为什么这道题选这个答案；可写 A、B、C、D 选项分析。" required /></label>
