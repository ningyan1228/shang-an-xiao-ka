...
const headerAliases: Record<(typeof csvHeaders)[number], string[]> = {
  category_slug: ['category_slug', '分类标识'], topic_slug: ['topic_slug', '专题标识'], slug: ['slug', '题目标识'], status: ['status', '状态'], question_type: ['question_type', '题型'], question: ['question', '题干'], option_a: ['option_a', '选项A'], option_b: ['option_b', '选项B'], option_c: ['option_c', '选项C'], option_d: ['option_d', '选项D'], correct_option: ['correct_option', '正确选项'], answer: ['answer', '标准答案'], explanation: ['explanation', '解析'], mnemonic: ['mnemonic', '记忆口诀'], mistake_tip: ['mistake_tip', '易错提醒'], difficulty: ['difficulty', '难度'], is_free: ['is_free', '是否免费'], sort_order: ['sort_order', '排序'], question_image_filename: ['question_image_filename', '题干图片文件名'], answer_image_filename: ['answer_image_filename', '答案图片文件名']
};
...
export function parseCsv(text: string): ImportRow[] {
  ...
  return lines.slice(1).map((line, index) => {
    ...
    if (!['choice', 'recall'].includes(row.question_type)) errors.push('题型无效');
    if (row.question_type === 'choice' && !['A', 'B', 'C', 'D'].includes(row.correct_option)) errors.push('选择题正确选项无效');
